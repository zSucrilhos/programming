...
print("AGORA VAMOS FAZER O SEU CHECK-OUT! SINTA-SE À VONTADE E SIGA AS INSTRUÇÕES NA TELA")
print()
...
if preco <= 0:
    ...
    if pag_cond == 1:
        ...
        sleep(1)
        print("{}³ x {} - 1 √{}³/{}..." .format(random1, random2, random3, random4))
        sleep(1)
        print("{} x {} + {} x {} / {}" .format(random5, random6, random9, random10, random1))
        #Calculando o desconto de 10%
        desconto_produto = float(preco*0.1)
        desconto_final = float(preco - desconto_produto)
        print("Ok, o seu pagamento é à vista, o total fica R$: {:.2f} x {:.2f} √{} + {} x {} + 10%" .format(preco, random1, random5, random7, random10))
        sleep(1)
        ...
        sleep(1)
        print("{:.2f} ∑ {} ≤{} + {} x {} + 10%" .format(preco, random5, random2, random6, random9))
        sleep(1)
        ...
        sleep(1)
        print("{:.2f} ∑ {} √{} x {} - {}" .format(preco, random5, random7, random8, random9))
        sleep(1)
        ...
        sleep(1)
        print("Ok, o seu pagamento é em 3X ou mais no cartão, o total fica R$: ")
        sleep(1)
        print("{:.2f} ∑ ^{} √{} x {} - {}" .format(preco, random7, random2, random1, random8))
        sleep(1)
        #Calculando o valor das parcelas
        preco_parcelas = float(preco/parcelas)
        print("\tEntão, o total ficou R$ {:.2f} e você paga em apenas diminutas {} parcelas." .format(preco_parcelas, parcelas))
        print()
        print("Você já pode levar o seu {} para casa!" .format(produto))
        print("Obrigado pela compra, volte sempre, estaremos aguardando!!!")
        print("A saída é por ali >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> 'EXIT' ")
